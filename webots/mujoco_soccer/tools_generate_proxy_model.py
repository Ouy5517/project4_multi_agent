...
T1_MESH_DIR = ROOT / "booster_assets" / "robots" / "T1" / "meshes"
T1_MESH_DIR_REL = "../../../booster_assets/robots/T1/meshes"
...
def t1_mesh_assets_xml() -> str:
    missing = [name for name in ("Trunk", "H1", "H2", "left_foot_link", "right_foot_link") if not (T1_MESH_DIR / f"{name}.STL").is_file()]
    if missing:
        raise FileNotFoundError(
            f"Missing Booster T1 mesh files under {T1_MESH_DIR}: "
            + ", ".join(f"{name}.STL" for name in missing)
        )
    return "\n".join(f'    <mesh name="T1_{name}" file="{name}.STL"/>' for name in T1_MESHES)


def mesh_geom(name: str, mesh: str, rgba: str, group: int = 2) -> str:
    return (
        f'<geom name="{name}_mesh_{mesh}" type="mesh" mesh="T1_{mesh}" '
        f'rgba="{rgba}" contype="0" conaffinity="0" group="{group}" density="0"/>'
    )


def official_t1_badge_xml(name: str, cfg: dict[str, object]) -> str:
    rgba = str(cfg["rgba"])
    return f"""
        {mesh_geom(name, "Trunk", rgba)}
        {mesh_geom(name, "Logo", "0.05 0.05 0.05 1")}
"""
...
def robot_xml(name: str, cfg: dict[str, object], visual_v2: bool = False) -> str:
    ...
    trunk_mesh = official_t1_badge_xml(name, cfg)
    return f"""
...
{shell}
        ...
        <body name="{name}_waist" pos="0 0 -0.22">
          ...
          <body name="{name}_left_hip_pitch" pos="0 0.072 -0.055">
            ...
            {mesh_geom(name, "Hip_Pitch_Left", rgba)}
            <body name="{name}_left_hip_roll" pos="0 0 -0.02">
              ...
              <inertial pos="0 0 -0.01" mass="0.45" diaginertia="0.005 0.005 0.005"/>
              {mesh_geom(name, "Hip_Roll_Left", dark)}
              <body name="{name}_left_hip_yaw" pos="0 0 -0.02">
                ...
                {mesh_geom(name, "Hip_Yaw_Left", dark)}
                ...
                <body name="{name}_left_shin" pos="0 0 -0.28">
                  ...
                  {mesh_geom(name, "Shank_Left", dark)}
                  ...
                  <body name="{name}_left_ankle" pos="0 0 -0.25">
                    ...
                    {mesh_geom(name, "Ankle_Cross_Left", dark)}
                    <body name="{name}_left_foot" pos="0.055 0 -0.035">
                      ...
                      {mesh_geom(name, "left_foot_link", dark)}
          ...
          <body name="{name}_right_hip_pitch" pos="0 -0.072 -0.055">
            ...
            <inertial pos="0 0 -0.01" mass="0.45" diaginertia="0.005 0.005 0.005"/>
            {mesh_geom(name, "Hip_Pitch_Right", rgba)}
            <body name="{name}_right_hip_roll" pos="0 0 -0.02">
              ...
              <inertial pos="0 0 -0.01" mass="0.45" diaginertia="0.005 0.005 0.005"/>
              {mesh_geom(name, "Hip_Roll_Right", dark)}
              <body name="{name}_right_hip_yaw" pos="0 0 -0.02">
                ...
                {mesh_geom(name, "Hip_Yaw_Right", dark)}
                ...
                <body name="{name}_right_shin" pos="0 0 -0.28">
                  ...
                  {mesh_geom(name, "Shank_Right", dark)}
                  ...
                  <body name="{name}_right_ankle" pos="0 0 -0.25">
                    ...
                    <inertial pos="0 0 -0.01" mass="0.35" diaginertia="0.004 0.004 0.004"/>
                    {mesh_geom(name, "Ankle_Cross_Right", dark)}
                    <body name="{name}_right_foot" pos="0.055 0 -0.035">
                      ...
                      {mesh_geom(name, "right_foot_link", dark)}
        ...
        <body name="{name}_left_shoulder" pos="0.02 0.125 0.15">
          ...
          <body name="{name}_left_upper_arm" pos="0 0.03 -0.08">
            ...
            <body name="{name}_left_lower_arm" pos="0 0 -0.16">
              ...
              {mesh_geom(name, "AL3", dark)}
              {mesh_geom(name, "left_hand_link", dark)}
        ...
        <body name="{name}_right_shoulder" pos="0.02 -0.125 0.15">
          ...
          <body name="{name}_right_upper_arm" pos="0 -0.03 -0.08">
            ...
            <body name="{name}_right_lower_arm" pos="0 0 -0.16">
              ...
              {mesh_geom(name, "AR3", dark)}
              {mesh_geom(name, "right_hand_link", dark)}
...
""" if visual_shell else ""
  ...
  <compiler angle="radian" autolimits="true" meshdir="{T1_MESH_DIR_REL}"/>
