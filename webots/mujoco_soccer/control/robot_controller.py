...
from mujoco_soccer.control.joint_trajectory import JointTrajectoryState
...
class RobotController:
    ...
    joint_trajectory: JointTrajectoryState = field(default_factory=JointTrajectoryState)
    ...
    def start_joint_trajectory(self, action: str) -> None:
        self.joint_trajectory.start(action)
    ...
    def update(self, data: mujoco.MjData, sim_time: float, dt: float) -> bool:
        ...
        self.joint_trajectory.step(dt)

        # 兼容: 外部设置的 push_pose 与踢腿相位包络取较大值
        swing_push = self.kick_swing.push_pose_equivalent()
        trajectory_push = self.joint_trajectory.push_pose_equivalent()
        effective_push = max(self.push_pose, swing_push, trajectory_push)
        ...
        self.moving = (not arrived) or abs(effective_push) > 1e-4 or self.kick_swing.active or self.joint_trajectory.active
        kick_progress = self.kick_swing.progress if self.kick_swing.active else -1.0
        joint_offsets = self.kick_swing.joint_offsets()
        joint_offsets.update(self.joint_trajectory.joint_offsets())
        self.gait.update(
            data,
            sim_time,
            dt,
