...
class ConcurrentMatch:
    ...
    def _apply_controllers(self) -> None:
        ...
        for robot in ROBOTS:
            ...
            if command.kick_action is not None and command.kick_target is not None:
                ball = self._ball_xy()
                start, push = plan_push(robot, ball, command.kick_target, target.max_speed, 0.05, command.kick_action)
                x, y = self._robot_positions()[robot]
